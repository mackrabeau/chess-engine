...
void test_en_passant_legality() {
    ...
    for (int i = 0; i < legalMoves.getNumMoves(); ++i)
        if (legalMoves.getMove(i) & FLAG_EP_CAPTURE) foundEnPassant = true;
...
void test_en_passant_illegality() {
    ...
    Move legalMoves = moveGen.generateAllLegalMoves(board);
    for (int i = 0; i < legalMoves.getNumMoves(); ++i){
        assert(!(legalMoves.getFlags(i) == FLAG_EP_CAPTURE) && "En passant should NOT be legal");
    }
}
...
void test_castling_rights() {
    ...
    for (int i = 0; i < legalMoves.getNumMoves(); ++i) {
        U16 move = legalMoves.getMove(i);
        if ((move & FLAG_KING_CASTLE) && !(move & FLAG_DOUBLE_PAWN_PUSH)) foundKingCastle = true;
        if ((move & FLAG_KING_CASTLE) && (move & FLAG_DOUBLE_PAWN_PUSH)) foundQueenCastle = true;
    }
...
void test_pawn_promotions() {
    ...
    for (int i = 0; i < legalMoves.getNumMoves(); ++i)
        if (legalMoves.getMove(i) & FLAG_PROMOTION) foundPromotion = true;
...
U64 perft(Game& game, int depth) {
    ...
    U64 nodes = 0;
    Move legalMoves = game.moveGen.generateAllLegalMoves(game.board);
    for (int i = 0; i < legalMoves.getNumMoves(); ++i) {
        Board nextBoard = game.makeMove(legalMoves.getMove(i));
...
int main() {
    ...
    U64 nodes = perft(game, 3);
    std::cout << "Perft (3): " << nodes << std::endl;
